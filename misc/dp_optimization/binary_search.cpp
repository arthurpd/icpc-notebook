...
/*
        Binary Search Optimization for DP:
                ...
                In this case, i is the number of elements to partition and
   j is the number of partitions allowed.

                Let opt[i][j] be the values of k which minimize the
   function. (in case of tie, choose the smallest) To apply this
   optimization, you need opt[i][j] <= opt[i+1][j]. That means the when you
   add an extra element (i + 1), your partitioning choice will not be to
   include more elements than before (e.g. will no go from choosing [k, i]
   to [k-1, i+1]). This is usually intuitive by the problem details.
...
void compute(int j, int l, int r, int low_opt, int high_opt) {
  ...
  if (l <= mid - 1) compute(j, l, mid - 1, low_opt, opt);
  if (mid + 1 <= r) compute(j, mid + 1, r, opt, high_opt);
}
int main(void) {
  scanf("%d %d", &n, &m);
  for (int i = 1; i <= n; i++)
  ...
  // 		tab[i][j] = inf;
  // 		for (int k = 0; k < i; k++)
  // 			tab[i][j] = min(tab[i][j], tab[k][j-1] + c(k +
  // 1,i);
  // 	}
  for (int j = 1; j <= m; j++) compute(j, 1, n, 0, n - 1);
  cout << tab[n][m] << endl;
}
