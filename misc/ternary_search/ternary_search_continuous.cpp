/*
        ...
        Constraints:
                ...
                and strictly increasing in a interval [x2, top]. x1 <= x2
   are arbitrary values where [x1, x2] is a plateau of optimal solutions.

        Usage:
                Call the function passing a lambda expression or function
   f. If there are multiple possible solutions, assume that an arbitrary
   one in the plateau is returned.

        Author: Arthur Pratti Dadalto
*/
template <typename F>
double ternary_search(const F &f, double bot = -1e9, double top = 1e9,
                      double eps = 1e-9) {
  while (top - bot > eps) {
