...
struct node {
  ...
  ~node() {
    for (auto child : next_node) delete child.second;
  }
  // Lenght of current edge
  int len() { return right - left + 1; }
  // Convinient way to find kid
  node *next(char c) {
    if (next_node.count(c)) return next_node[c];
    return NULL;
  }
};
struct suffix_tree {
  ...
  suffix_tree(string _s, char _dummy = '$') {
    ...
    for (int j = 0; j < (int)s.size(); j++) {
      ...
      while (i <= j) {
        ...
        if (cur_dist == cur_node->len() - 1 and
        ...
        // Caso 3: s[i...j-1] ends in an edge
          ...
          node *mid = new node(cur_node->left, cur_node->left + cur_dist,
                               cur_node->parent);
          ...
          if (need_suffix != NULL) need_suffix->suffix = mid;
  ...
  char get_char(node *cur, int ind) { return s[cur->left + ind]; }
  // Optional
  bool verify_substring(string sub) {
    ...
    for (char c : sub) {
      if (cur_dist < cur_node->len() - 1) {
        if (get_char(cur_node, cur_dist + 1) != c) return false;
        cur_dist++;
      } else {
        if (cur_node->next(c) == NULL) return false;
        cur_node = cur_node->next(c);
        cur_dist = 0;
      }
    }
