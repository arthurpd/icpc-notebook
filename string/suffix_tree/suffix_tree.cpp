/*
	Suffix Tree:
        A compressed trie is a trie where all reduntant nodes are eliminated by allowing edges to hold substrings.
        A Suffix Tree is a Trie containig all the suffixes of a certain string S.
        Using a dummy character in the end of S gurantees that all suffixes end in leafs, and vice-vers.
        This code builds the Suffix Tree in O(|S|*lg|Alph|) time and O(|S|) memory (where |Alph| is the size of the alphabet).
	...
	Usage:
		Create an object Suffix Tree st passing the string as argument, and optionally the dummy character as second argument.
        "verify_substring(P)" checks in O(|P|) if P is a substring of S.
...
struct node{
    ...
    //Keeps left and right index of edge's substring in S. (may not be the same occurrence one, see "aba$"); 
    int left, right;
    //*parent points to parent node
    //*suffix points to node corresponding to [left+1...right] (by the end of the process exists for every node other than the root)
    node *parent, *suffix;
    //next_node[c] points to the kid of current node whose edge begins with character c (only one by character).
...
struct suffix_tree{
    ...
    suffix_tree(string _s, char _dummy = '$')
    {
        ...
        //In the beginning of iteration i,j, node cur_node in [left...cur_dist] represents [i...j-1]
        //need_suffix points to node that doesn't have a suffix yet (at most one at a time, for at most one iteration of i)
        node *cur_node = root;
        int cur_dist = -1, i = 0;

        //Invariants:        
        //  *At the beginning of step i,j, s[i...j-1] and all of it's suffixes are inserted in the suffix trie
        //  *At the beggining of step i,j, cur_node[cur_dist] is the end point representing s[i...j-1]
        //  *At any increment of i, at most one node doesn't have "suffix" field defined, and it's stored in "need_suffix"
        for(int j = 0; j < (int)s.size(); j++){
            ...
            while(i <= j){
                ...
                //If now we have to take one more char from the edge, take it
                ...
                //Caso 3: s[i...j-1] ends in an edge
                    ...
                    if(cur_node != root){
                        ...
                        g = i + 1;
                    //Initially cur_node points to node [i+1 ... g-1]
