...
/*
        Prefix Function and KMP:
                ...
                algorithms don't depend on the alphabet size, they only
...
// pi[i] is the size of its (the prefix's) largest proper prefix
...
vector<int> prefix_function(T s) {
  ...
  for (int i = 2; i <= s.size(); i++) {
    int j = pi[i - 1];  // j is the size of the candidate prefix
                        // to expand.
    while (j > 0 && s[j] != s[i - 1])  // While we still have a
                                       // candidate prefix and it
                                       // can't be expanded.
      j = pi[j];                       // Go to the next candidate prefix.
    // If candidate prefix can be expanded, do it. Otherwise,
    // there is no prefix that is also a suffix.
    pi[i] = s[j] == s[i - 1] ? j + 1 : 0;
  }
  return pi;
}
// Returns a sorted list of all positions in the text string where
// begins an ocurrence of the key string.
// e.g. kmp("aabaaab", "aab") returns {0, 4}.
template <class T>
vector<int> kmp(T text, T key) {
  vector<int> retv;
  vector<int> pi = prefix_function(key);
  // There is no need to have the entire text in memory, you could
  // do this char by char.
  for (int i = 0, match = 0; i < text.size(); i++) {
    // match stores the size of the prefix of the key which is a
    // suffix of the current processed text.
    while (match > 0 && text[i] != key[match]) match = pi[match];
    if (text[i] == key[match]) match++;
    if (match == key.size()) {
      retv.push_back(i - match + 1);
      match = pi[match];  // To avoid access to key[key.size()]
                          // in next iteration.
    }
  }
  return retv;
