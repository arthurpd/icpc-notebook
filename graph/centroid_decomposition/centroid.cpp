...
/*
        Centroid Decomposition:
                Solve tree problems by divide and conquer splitting the
   tree repeatedly on centroid. Centroid is the vertex with smallest
   <largerst subtree>. O(n log n if process is O(sz))
        ...
        Usage:
                ...
                subtree total size sz.
                        ...
                        sub_size[a][i] - subtree size for edge a ->
   graph[a][i] (considering only non-blocked parts).

                if process can be O(sz + h * log) where h is subtree height
   it is a lot better constant than O(sz * log)
...
void decomp(int a, int sz) {
  ...
  block[a] = true;
  for (int i = 0; i < sz(graph[a]); i++)
    if (!block[graph[a][i]]) decomp(graph[a][i], sub_size[a][i]);
}
