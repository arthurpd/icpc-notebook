...
/*
        Finding bridges, articulation points and biconnected components in
                ...
                A biconnected component (or 2VCC) is a maximal subgraph
   where the removal of any vertex doesn't make the subgraph disconnected.
   In other words, it is a maximal 2-vertex-connected (2VC) subgraph.
    ...
    Useful theorems:
        ...
        Let G be a graph on at least 2 vertices. The following
            ...
            * (ii) any two vertices are in a cycle; (a cycle can't
                               repeat vertices)
            * (iii) any two edges are in a cycle and degree(G) >= 2;
        ...
        Let G be a graph on at least 3 vertices. The following
            ...
            * (iv) any two vertices are in a tour
                ...
                If G is 2-connected and not bipartite, all vertices belong
   to some odd cycle. And any two vertices are in a odd cycle (not really
   proven).
        ...
        A strong orientation of a given bridgeless undirected graph
                may be found in linear time by performing a depth first
   search of the graph, orienting all edges in the depth first search tree
   away from the tree root, and orienting all the remaining edges (which
   must necessarily connect an ancestor and a descendant in the depth first
   ...
   maximum vertex label (n), the graph adjacency list (graph) and a
   callback f to apply on the biconnected components. Afterwards, art[i] ==
   true if i is an articulation point. If the pair {a, i} is on the bridges
   list, then the edge {a, graph[a][i]} is a bridge. The callback must
   receive a vector of edges {a, b} that are in the same biconnected
   component. Remember that for a single vertex, the biconnected callback
                will not be called.
...
struct apb {
  ...
  int id;
  template <class F>
  apb(int a, int n, vector<int> graph[], const F &f)
