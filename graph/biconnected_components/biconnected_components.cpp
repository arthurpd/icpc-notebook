...
/*
	Finding bridges, articulation points and biconnected components in O(V + E):
    ...
    Useful theorems:
        ...
        descendant in the depth first search tree) from the descendant to the ancestor.
...
void dfs(int a, int p, const F &f)
{
	...
	for (int i = 0; i < sz(graph[a]); i++)
	{
		...
		else if (graph[a][i] != p && num[graph[a][i]] < num[a]) // Back edge.
        {
