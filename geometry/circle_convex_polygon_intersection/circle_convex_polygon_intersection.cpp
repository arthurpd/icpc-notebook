...
double circle_convex_polygon_intersection(const vector<point> &p,
                                          circle c) {
  double retv = 0;
  int n = sz(p);
  for (int i = 0; i < n; i++) {
    ...
    if (c.intersect(l, res) == 2) {
      if (s.dist(res.first) < EPS) bd.push_back(res.first);
      if (s.dist(res.second) < EPS) bd.push_back(res.second);
    }
    sort(bd.begin() + 1, bd.end(), [&bd](point lhs, point rhs) {
      return (lhs - bd[0]).dist2() < (rhs - bd[0]).dist2();
    });
    if (bd.size() == 2) {
      if ((bd[0] - c.center).dist() < c.r + EPS &&
          (bd[1] - c.center).dist() <
              c.r + EPS)  // Segment completely inside.
