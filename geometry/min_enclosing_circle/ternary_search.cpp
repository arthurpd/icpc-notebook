...
/*
        Minimum Enclosing Circle:
                ...
                Runs in O(n * log^2((top - bot) / eps)) (in practice 2.5s
   at best for 10^5 points).
        ...
        Usage:
                ...
                higher value than necessary since the error in x affects
   the y value.
...
circle min_enclosing_circle(const vector<point> &p, double bot = -1e9,
  ...
  auto f1 = [&](double x) {
    auto f2 = [&](double y) {
      ...
      for (int i = 0; i < sz(p); i++)
        r = max(r,
                (p[i].x - x) * (p[i].x - x) + (p[i].y - y) * (p[i].y - y));
      return r;
    };
