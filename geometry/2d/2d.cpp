...
struct line {
  ...
  double dist(P p) {
    return (double)abs(a * p.x + b * p.y - c) /
           sqrt((double)(a * a + b * b));
  }
  ...
  P reflect(P p) {
    P res;
    res.x = ((b * b - a * a) * p.x - 2 * a * b * p.y + 2 * a * c) /
            (a * a + b * b);
    res.y = ((a * a - b * b) * p.y - 2 * a * b * p.x + 2 * b * c) /
            (a * a + b * b);
    return res;
  }
};
...
struct circle {
  ...
  int intersect(circle rhs, pair<P, P> &retv) {
    ...
    int num = rhs.intersect(
        line<T>(2 * rhs.center.x, 2 * rhs.center.y,
                rhs.center.x * rhs.center.x + rhs.center.y * rhs.center.y +
                    r * r - rhs.r * rhs.r),
        retv);
  ...
  pair<P, P> tangents(P p) {
    ...
    return {center + p * k1 + p.perp() * k2,
            center + p * k1 - p.perp() * k2};
  }
  ...
  void outter_tangents(circle other, pair<P, P> &retV1,
                       pair<P, P> &retV2) {
    ...
    retV2.first = P(other.center.x + other.r * cos(a3),
    ...
    // found.
    if (abs((center - other.center).dist() + min(r, other.r) -
            max(r, other.r)) < EPS) {
  ...
  void inner_tangents(circle other, pair<P, P> &retV1, pair<P, P> &retV2) {
    ...
    double angle = acos(u);
    P vec = cp - center;
    retV1 = {center + vec.rotate(angle) * u,
             center + vec.rotate(-angle) * u};
    // find points for other circle
    u = other.r / (other.center - cp).dist();
    angle = acos(u);
    vec = cp - other.center;
    retV2 = {other.center + vec.rotate(angle) * u,
             other.center + vec.rotate(-angle) * u};
    // In the case there is one tangent point (and circles are
...
circle<double> circumcircle(const point<double> &A, const point<double> &B,
                            const point<double> &C) {
  circle<double> retv;
  point<double> a = C - B, b = C - A, c = B - A;
  retv.r = a.dist() * b.dist() * c.dist() / abs(c.cross(b)) / 2;
  retv.center =
      A + (b * c.dist2() - c * b.dist2()).perp() / b.cross(c) / 2;
  return retv;
}
