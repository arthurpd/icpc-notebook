...
struct point
{
	...
	ll cross(point p1, point p2) { return (p1.x - x) * (p2.y - y) - (p2.x - x) * (p1.y - y); }
...
void convex_hull(vector<point> p, vector<point> &upper, vector<point> &lower)
{
	...
	auto build = [&p](vector<point> &ch, ll tp) {
		...
		{
			while (ch.size() >= 2 && tp * ch[sz(ch) - 2].cross(ch.back(), p[i]) >= 0)
				ch.pop_back();
			ch.push_back(p[i]);
		}
	};
...
bool point_in_ch(point o, vector<point> &upper, vector<point> &lower)
{
	...
	auto check = [o](vector<point> &ch, ll tp) {
		int i = lower_bound(ch.begin(), ch.end(), o, [](point a, point b) { return a.x < b.x; }) - ch.begin();
