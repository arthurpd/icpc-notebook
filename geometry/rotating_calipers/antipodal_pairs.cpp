...
/*
        Antipodal pairs O(n):
                Uses rotating calipers techinique to find all antipodal
   pairs. Returned list will be such the the entire polygon lies between
                the line defined by (p[retv[i].first],
                                                     p[(retv[i].first + 1)
   % n]) and a parallel line passing by p[retv[i].second].
...
vector<pii> antipodal_pairs(const vector<point<ll>> &p) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    while (p[i].cross(p[(i + 1) % n], p[(j + 1) % n]) >
           p[i].cross(p[(i + 1) % n], p[j]))
      j = (j + 1) % n;
    retv.push_back({i, j});
    // If j + 1 is at the same distance as j, both pairs are
    // antipodal.
    if (p[i].cross(p[(i + 1) % n], p[(j + 1) % n]) ==
        p[i].cross(p[(i + 1) % n], p[j]))
