...
int main(void) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      ...
      if (!(c[i].center == c[j].center) &&
          c[i].intersect(c[j], intersections) > 0) {
    ...
    ans += simpsons(
        [&](double x) {
          ...
          for (int j = 0; j < v.size(); j++) {
            ...
            double h = sqrt(
                c[v[j].second.first].r * c[v[j].second.first].r - d * d);
            double tmp =
                c[v[j].second.first].center.y - v[j].second.second * h;
        ...
        2 * max(1, int(((good_x[i + 1] - good_x[i]) / range_x) * total)),
        good_x[i], good_x[i + 1]);
  }
  printf("%.5lf\n", ans);
}
