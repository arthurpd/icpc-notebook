...
optimization_status solve_tab(mat<double> &tab, vector<int> &basic_var)
{
	...
	while ((entering_var = get_entering_var(tab)) != -1)
	{
		...
		vec<double>::linear_comb(tab[exiting_var_row], (1.0 / tab[exiting_var_row][entering_var]), tab[exiting_var_row], 0.0, tab[exiting_var_row]);

		// Gaussian elimination of the other rows.
		for (int i = 0; i < sz(tab); i++)
			if (i != exiting_var_row)
				if (abs(tab[i][entering_var]) > EPS)
					vec<double>::linear_comb(tab[i], 1.0, tab[exiting_var_row], -tab[i][entering_var], tab[i]);
...
// maximize c*x, s.t. a*x <ops> b. x >= 0.
optimization_status run_simplex(int num_constraints, int num_vars, mat<double> a, vec<op> ops, vec<double> b, vec<double> c, vec<double> &res, double &obj_val)
{
	...
	mat<double> tab(num_constraints + 1, num_vars + num_slack_variables + num_artificial_variables + 1);
	...
	for (int i = num_vars + num_slack_variables; i < num_vars + num_slack_variables + num_artificial_variables; i++)
		...
		if (basic_var[i] >= num_vars + num_slack_variables)
		{
			...
			for (int j = 0; j < sz(tab[i]) - 1 - num_artificial_variables; j++)
			{
				...
				if (j != basic_var[i] && abs(tab[i][j]) > EPS)
				{
					...
					vec<double>::linear_comb(tab[i], 1.0 / tab[i][j], tab[i], 0, tab[i]);
	...
	for (int i = 0; i < sz(tab[0]); i++)
		tab[0][i] = 0;
	for (int i = 0; i < num_vars; i++)
		tab[0][i] = -c[i];

	for (int i = 1; i < sz(tab); i++)
		vec<double>::linear_comb(tab[0], 1, tab[i], -tab[0][basic_var[i]], tab[0]);
