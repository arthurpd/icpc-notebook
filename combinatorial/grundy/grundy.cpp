...
/*
        Mex for Grundy Number O(N):
                ...
                loss condition).
                Then, for each state, find the MEX for the Grundy Numbers
   of reacheable states (i.e. the lowest Grundy Number not present). This
   will be the current state's Grundy Number. If you have many parallel
   games, you can find the equivalent Grundy number by doing XOR of
   individual Grundy Numbers. 0 equals losing position, any other value is
   a winning position.
...
int mex(vector<int> v) {
  ...
  for (int i = 0; i < v.size(); i++) {
    while (v[i] < v.size() && v[v[i]] != v[i]) swap(v[v[i]], v[i]);
  }
