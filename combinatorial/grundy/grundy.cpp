...
/*
	Mex for Grundy Number O(N):
		To calculate the Grundy Number for a set of states, first set terminal states' Grundy Number (zero if no move is a loss condition).
	...
	Constraints:
		The game must be symmetrical (same moves are available for both players); have perfect information (no hidden or random stuff); 
		be finite (no loops).

	Usage:
		For each state, save the Grundy Number of all reacheble states in a vector v, and pass as argument to mex().
