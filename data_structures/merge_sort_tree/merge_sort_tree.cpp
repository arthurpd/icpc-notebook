...
/*
        ...
        Usage:
                In this example, instead of building the merge sort tree
   with the given vector (e.g. {1, 5, 2, 6, 3, 7, 4}), we sort a vector of
   indices by their value in the vector (e.g. {0, 2, 4, 6, 1, 3, 5} for the
   vector above).

                This way, each node in the tree is responsible for a range
   of sorted elements in the vector and we can ask it how many of those
   have indices in the range [a, b].
*/
#define left(i) ((i) << 1)
...
struct merge_sort_tree {
  ...
  merge_sort_tree(vector<int> v) : v(v), val(4 * (sz(v) + 1)) {
    for (int i = 0; i < sz(v); i++) indices.push_back(i);
  ...
  int count_interval(int id, int a, int b) {
    return (int)(upper_bound(all(val[id]), b) -
                 lower_bound(all(val[id]), a));
  }
  int get(int id, int l, int r, int a, int b, int x) {
    if (l == r) return v[val[id].back()];
    int mid = (l + r) / 2;
  ...
  int kth(int a, int b, int k) { return get(1, 0, sz(v) - 1, a, b, k); }
};
