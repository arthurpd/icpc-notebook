...
struct nodes
{
	...
	void updateb(int l, int r, int a, ll x, nodes *o, nodes *p)
	{
		...
		else
		{
			int mid = (l + r) / 2;
			if (a <= mid)
				(left ? left : (left = new nodes()))->updateb(l, mid, a, x, o ? o->left : NULL, p ? p->left : NULL);
			else
				(right ? right : (right = new nodes()))->updateb(mid + 1, r, a, x, o ? o->right : NULL, p ? p->right : NULL);
	...
	ll get(int l, int r, int a, int b)
	{
		...
		else
		{
			...
			if (b <= mid)
				...
				return __gcd(left ? left->get(l, mid, a, mid) : 0, right ? right->get(mid + 1, r, mid + 1, b) : 0);
		}
	}
};
	...
	void update(int l, int r, int a, int b, ll x)
	{
		...
		else
		{
			...
			if (a <= mid)
				...
				(right ? right : (right = new nodef()))->update(mid + 1, r, a, b, x);

			val->updateb(0, m - 1, b, x, left ? left->val : NULL, right ? right->val : NULL);
		}
	}
	...
	ll get(int l, int r, int a, int b, int c, int d)
	{
		...
		else
		{
			...
			if (b <= mid)
				...
				return __gcd(left ? left->get(l, mid, a, mid, c, d) : 0, right ? right->get(mid + 1, r, mid + 1, b, c, d) : 0);
		}
	}
};
