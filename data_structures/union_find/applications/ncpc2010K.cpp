...
int main(void) {
  ...
  for (int i = 0; i < n; i++) {
    if (graph[i].size() == 2) {
      ans += acos(-1) - abs((p[graph[i][0].first] - p[i])
                                .angle(p[graph[i][1].first] - p[i]));
      diff[i] = 0;
      ...
      for (int j = 1; j < 4; j++) {
        ...
        if (3 + j == 6)
          ...
          k = 2, l = 3;
        double d1 =
            acos(-1) - abs((p[graph[i][0].first] - p[i])
                               .angle(p[graph[i][j].first] - p[i]));
        double d2 =
            acos(-1) - abs((p[graph[i][k].first] - p[i])
                               .angle(p[graph[i][l].first] - p[i]));
  ...
  for (int i = 0; i < n; i++) pi.push_back({diff[i], i});
