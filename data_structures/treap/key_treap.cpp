...
/*
        Treap:
                This treap implements something like a c++ set with
   additional operations: find the k-th element and count elements less
   than a given value.
...
int get_num(node *root) { return (root == NULL) ? 0 : root->num; }
...
node *insert(node *root, int x) {
  ...
  update_num(root);
  if (root->right && root->right->p > root->p) root = rotate_left(root);
  if (root->left && root->left->p > root->p) root = rotate_right(root);
  return root;
}
