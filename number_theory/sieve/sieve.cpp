...
/*
        Sieve of Eratosthenes:
                ...
                Also finds lp[i] for every i in [2, MAXP], such that lp[i]
   is the minimum prime factor of i. Particularly useful for factorization.
...
void init() {
  for (int i = 2; i <= MAXP; i++) {
    if (lp[i] == 0) p.push_back(lp[i] = i);
    for (int j = 0; j < (int)p.size() && p[j] <= lp[i] && i * p[j] <= MAXP;
         j++)
      lp[i * p[j]] = p[j];
  }
}
